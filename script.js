...
const forms = state => {
  ...
  Object(_checkNumInputs__WEBPACK_IMPORTED_MODULE_0__["default"])('input[name = "user_phone"]'); // phoneInputs.forEach(item=>{
  //     item.addEventListener('input', () => {
  //         item.value = item.value.replace(/\D/, '');
  //     });
  // });
  ...
  form.forEach(item => {
    item.addEventListener('submit', e => {
      ...
      const formData = new FormData(item); // ниже в условие добавляем функционал для 
...
const modals = () => {
  function bindModal(triggerSelector, modalSelector, closeSelector, closeClickOverlay = true) {
    ...
    const trigger = document.querySelectorAll(triggerSelector),
          ...
          scroll = calcScroll(); // windows - будет использоваться для закрытия всех открытых модальных окон
    ...
    trigger.forEach(item => {
      item.addEventListener('click', e => {
        ...
        modal.style.display = "block"; // указывается, чтобы скролл был только внутри модального окна

        document.body.style.overflow = "hidden"; // вместо верхнего варианта(работающего с нашими классами)подключаем
  ...
  function calcScroll() {
    let div = document.createElement('div'); // шаг 1 сами создаем скролл

    div.style.width = '50px';
    div.style.height = '50px';
    div.style.overflowY = 'scroll';
    div.style.visibility = 'hidden'; // шаг2 помещаем его на страницу и вычитаем его отсупы
